...
# import run_baseline
# import run_sample
# import run_save_callback
# import run_variable_episode
# import run_vectorized
# import run_multiaction
# import train_RL
...
class BoptestGymEnvTest(unittest.TestCase, utilities.partialChecks):
    ...
    def test_reset_fixed(self):
        '''
        Test that the environment can reset using a fixed start time and a specific warmup period.
        '''
        ...
        env.start_time         = 16*24*3600 # start: Jan.17
        env.warmup_period      = 24*3600

        obs, _ = env.reset()

        # Check values
        df = pd.DataFrame(data={'value': [obs]}, index=['obs_reset_fixed'])
        df.index.name = 'keys'
    ...
    def test_reset_random(self):
        ...
        excluding_periods = [(16*24*3600,  16*24*3600+14*24*3600),
                            (108*24*3600, 108*24*3600+14*24*3600)]
        env.excluding_periods = excluding_periods
        random.seed(seed)
        start_times = OrderedDict()
        # Reset two hundred times
        for i in range(200):
            obs, _ = env.reset()
        ...
        # Check values
        start_times_converted = {k: [tuple(v)] for k, v in start_times.items()}
        df = pd.DataFrame.from_dict(start_times_converted, orient='index', columns=['value'])
    ...
    def test_get_reward_default(self):
        ...
        obs, _, rew = run_baseline.run_reward_default(plot=False)   # 可能有问题
    ...
    def partial_test_RL(self, algorithm='DQN', mode='load', episode_length_test=14*24*3600,
...
if __name__ == '__main__':
    ...
    test_instance = BoptestGymEnvTest()
    test_instance.setUp()
    test_instance.partial_test_RL()
