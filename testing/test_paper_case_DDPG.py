...
def train_RL(algorithm          = 'DDPG' ,
             start_time_tests    = [(23-7)*24*3600, (115-7)*24*3600],  #  测试的起始时间点（单位：秒）：模拟年的第16天-模拟年的第108天
             episode_length_test = 14*24*3600, #  每次测试的持续时间
    ...
    if mode == 'train':
        model = DDPG( 'MlpPolicy',
                    ...
                    buffer_size=100000,
                    # buffer_size=365 * 24,   # 经验回放缓冲区大小（1年的小时数）
        ...
        # 使用专家轨迹进行预训练（Pretraining）的功能，是模仿学习（Imitation Learning）的一种应用
...
#
# env, model, start_time_tests, log_dir = train_RL(algorithm='DQN', mode='train', case='D', training_timesteps=300, render=render,
#                                                  expert_traj=os.path.join('trajectories', 'expert_traj_cont_28.npz'))
#
...
#
# # 调用测试函数
# obs, act, rew, kpi = test_event(env,model, start_time_tests, episode_length_test, warmup_period_test, log_dir, save_to_file, plot)
#
#
#
