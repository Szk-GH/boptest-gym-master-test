...
from test_and_plot import plot_results, test_agent
...
class BoptestGymEnv(gym.Env):
    ...
    def __init__(self, 
        ...
        if any([obs in self.all_predictive_vars for obs in observations]):
            ...
            # Do a sanity check
            if self.predictive_period is not None and self.predictive_period < 0:
            ...
            if self.is_predictive:
                # 如果未设置 predictive_period，默认为 0（仅当前时刻）
                predictive_period = self.predictive_period if self.predictive_period is not None else 0
                self.pred_n = int(predictive_period / self.step_period) + 1
            ...
            predictive_period = self.predictive_period if self.predictive_period is not None else 0
            self.end_year_margin = self.max_episode_length + predictive_period
